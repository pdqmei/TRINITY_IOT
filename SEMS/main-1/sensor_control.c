...
#include "driver/i2c.h" // Thêm thư viện I2C
#include "hal/adc_types.h" // Thêm để đảm bảo ADC channel, atten, bitwidth được định nghĩa chính xác.
...
int read_mq9_raw(void)
{
    ...
    ESP_LOGI(TAG, "💨 MQ-9 ADC Raw: %d", adc_raw);
    return adc_raw;
}
float read_mq9_ppm_from_raw(int adc_raw)
{
    int voltage_mv = 0;
...
sensor_data_t read_all_sensors(void)
{
    ...
    // Đọc MQ-9
    int raw = read_mq9_raw();
    data.co2_level = raw;
    data.co2_ppm = read_mq9_ppm_from_raw(raw);
    return data;
}