...
export function updateSensorFromMQTT(topic, data) {
    // topic: smarthome/kitchen/sensors/temp hoặc /humi hoặc /co2
    const parts = topic.split("/");
    const room = parts[1];
    const sensorName = parts[3]; // "temp", "humi", "co2"

    // ⚠️ OPTION: Bỏ comment dòng dưới nếu chỉ muốn hiển thị phòng đang xem
    ...
    // 1️⃣ Update Firebase (để lưu lịch sử / sync)
    // Sử dụng đúng đường dẫn của phòng trong topic
    const dbPath = `smarthome/${room}/sensors/${sensorName}`;
    update(ref(db, dbPath), {
        value: data.value,
        ts: data.ts || Date.now()
    });
    ...
    console.log(`✅ MQTT → Firebase sensor ${sensorName}: ${data.value}`);
}
...
async function syncSensorsFromFirebase() {
    ...
    try {
        const snapshot = await get(ref(db, sensorsPath));
        const sensorsData = snapshot.val();

        if (!sensorsData) {
            console.log(`Không có dữ liệu sensors cho phòng ${currentRoom}`);
            return;
        }

        const sensors = [
            { name: 'temp', elementId: 'tempValue' },      // ✅ Sửa lại
            { name: 'humi', elementId: 'humiValue' },      // ✅ Sửa lại
            { name: 'co2', elementId: 'co2Value' }
        ];

        sensors.forEach(sensor => {
            const data = sensorsData[sensor.name];
            if (data && document.getElementById(sensor.elementId)) {
                ...
                console.log(`Sensor ${sensor.name} updated UI: ${data.value}`);
            }
        });
